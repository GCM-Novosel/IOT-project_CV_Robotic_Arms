...
while True:
    ...
    if lmList != []:
        ...
        thumb = np.interp(thumb, [0.4, 1.0], [80, 0])      #normalizing the range of values according to each finger's geometry
        index = np.interp(index, [0.9, 1.7], [125, 0])
        middle = np.interp(middle, [0.8, 1.9], [100, 0])
        ring = np.interp(ring, [0.7, 1.8], [120, 0])
        pinkie = np.interp(pinkie, [0.6, 1.6], [135, 0])
        ...
        pin9.write(int(pinkie))
        pin10.write(int(ring))
        pin11.write(int(middle))
        pin12.write(int(index))
        pin13.write(int(thumb))
